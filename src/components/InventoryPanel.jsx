...
import { Package, Search, Plus, Save, Edit2, Trash2, Loader2, RefreshCw, X, Filter, ChevronLeft, ChevronRight, Warehouse, ArrowUpDown, Settings, Check } from 'lucide-react';
...
const InventoryPanel = ({ user, mode = 'manage' }) => {
  ...
  const [bodegasList, setBodegasList] = useState([]); // 🔥 Lista real de bodegas
  ...
  const handleSaveEditBodega = async (bod) => {
      ...
      setBodegaLoading(true);
      try {
          // Actualizar nombre en tabla bodegas
          const { error } = await supabase.from('bodegas').update({ nombre: newName }).eq('id', bod.id);
          if (error) throw error;
          
          // Actualizar en cascada los productos que estaban en la bodega vieja
  ...
  const handleDeleteBodega = async (bod) => {
      ...
      setBodegaLoading(true);
      try {
          // Verificar si hay productos dentro
