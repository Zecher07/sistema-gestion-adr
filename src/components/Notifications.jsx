...
const Notifications = ({ 
  ...
  const [pendingVales, setPendingVales] = useState([]); // 🔥 ESTADO PARA VALES PENDIENTES
  ...
  useEffect(() => {
    const fetchPendingVales = async () => {
      ...
      try {
        ...
        console.error("Error cargando vales pendientes para notificaciones", error);
      }
    };
    ...
    const interval = setInterval(fetchPendingVales, 10000);
  ...
  // COMBINAR CONTADORES (Tareas + Eventos Rápidos + Vales Pendientes)
  ...
  // Redirigir a la pestaña de Vales
  const handleGoToVales = () => {
      setIsOpen(false);
      if (onViewChange) {
          onViewChange('vales');
      }
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <Bell className={`h-6 w-6 ${realtimeEvents.length > 0 || pendingVales.length > 0 ? 'text-blue-600 animate-pulse' : 'text-slate-600'}`} />
        {totalCount > 0 && (
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-xl border border-slate-200 z-50 overflow-hidden flex flex-col max-h-[85vh]"
          >
            ...
            <div className="overflow-y-auto flex-1">
              ...
              {/* --- SECCIÓN NUEVA: VALES PENDIENTES DE APROBACIÓN (SOLO ADMIN) --- */}
              {isAdmin && pendingVales.length > 0 && (
                  <div className="border-b border-red-100">
                      <div className="bg-red-50 text-xs font-bold text-red-800 px-3 py-1.5 uppercase tracking-wider">
                          Vales por Aprobar ({pendingVales.length})
                      </div>
                      {pendingVales.map(vale => (
                          <div key={vale.id} className="p-3 hover:bg-red-50/50 transition-colors flex items-start gap-3 group border-b border-slate-100 last:border-0">
                              <div className="mt-1"><Receipt className="h-4 w-4 text-red-500" /></div>
                              <div className="flex-1 min-w-0 cursor-pointer" onClick={handleGoToVales}>
                                  <p className="text-sm font-bold text-slate-800">Retiro de {vale.vendedor}</p>
                                  <p className="text-xs text-slate-600 mt-0.5 truncate">{vale.concepto} • <span className="font-bold text-red-600">${Number(vale.monto).toFixed(2)}</span></p>
                              </div>
                              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <Button variant="ghost" size="icon" className="h-7 w-7 text-red-600 hover:text-red-700 hover:bg-red-100" onClick={handleGoToVales} title="Ir a Vales">
              ...
              {/* --- SECCIÓN 3: TAREAS DE TRABAJO (PENDIENTES) --- */}
              {workItems.length > 0 ? (
                <div className="divide-y divide-slate-100">
                  <div className="bg-slate-100 text-[10px] font-bold text-slate-500 px-3 py-1 uppercase tracking-wider">
                      Tus Órdenes Pendientes ({workItems.length})
                  </div>
                  {workItems.map(item => (
                    <div key={item.id} className="p-3 hover:bg-slate-50 transition-colors flex items-start gap-3 group">
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-blue-600 hover:text-blue-700 hover:bg-blue-50" onClick={() => handleOpenOrder(item)} title="Ver detalles">
                            <ExternalLink className="h-3.5 w-3.5" />
                          </Button>
                        </div>
                    </div>
                ...
                /* Mensaje si no hay nada en absoluto */
                workItems.length === 0 && realtimeEvents.length === 0 && pendingVales.length === 0 && (
                    <div className="p-8 text-center text-slate-400 text-sm">
                        <p>¡Todo al día! 🎉</p>
                        <p className="text-xs mt-1">No tienes tareas ni notificaciones pendientes.</p>
                    </div>
                )
              )}
            </div>

            <div className="bg-blue-50 p-3 text-center border-t border-blue-100 hover:bg-blue-100 transition-colors cursor-pointer group shrink-0" onClick={handleViewAll}>
